...
router.post('/', async (req: Request<EmployeeParams>, res: Response) => {
  ...
  if (!type || !startDate || !endDate || days == null) {
    return res.status(400).json({ error: 'Les champs type, startDate, endDate et days sont requis' });
  }

  // Valider le nombre de jours
  const parsedDays = typeof days === 'number' ? days : parseFloat(days);
  if (!Number.isFinite(parsedDays) || parsedDays <= 0) {
    return res.status(400).json({ error: 'Le nombre de jours doit être un nombre positif' });
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith('{')) {
      try {
        ...
        // Continuer avec la gestion d'erreur générique
      }
    }
